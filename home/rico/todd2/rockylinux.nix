{pkgs, ...}: {
  home.packages = [
    (pkgs.writeShellScriptBin "rockylinux" ''
      ...
      PRE_INIT_HOOKS="\"dnf -y install dnf-plugins-core && dnf install -y 'dnf-command(config-manager)' && dnf config-manager --enable crb devel && dnf -y install epel-release redhat-lsb-core tcsh gtk4 libserf libxcrypt-compat\""
